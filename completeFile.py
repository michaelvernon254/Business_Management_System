...
def register():
    if request.method == "POST":
        ...
        password = request.form["password"]
        password = generate_password_hash(password)
        User.create(names = names, workerid = workerid, position = position, email = email, password = password)
...
def login():
    if request.method == "POST":
        ...
        try:
            ...
            flash("Wrong Username or Password. It could be that the user does not exist!")
    return render_template("login.html")
